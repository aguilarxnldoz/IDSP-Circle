...
export default async function HomePage() {
	...
	// For guests, show public content (popular circles and recent public albums)
	// For authenticated users, show personalized feed
	let userCirclesFormatted: any[] = [];
	let suggestedCircles: any[] = [];
	let feedAlbums: any[] = [];
	...
	if (!isGuest) {
		...
		userCirclesFormatted = userCircles.map(membership => membership.circle);
		...
		feedAlbums = await prisma.album.findMany({
			where: {
				OR: [
					{
						// User's own albums
		...
		// Guest mode: Show popular public circles and recent public albums
		suggestedCircles = await prisma.circle.findMany({
			where: {
				isPrivate: false,
			},
		...
		// Show recent public albums (not in private circles)
		feedAlbums = await prisma.album.findMany({
			where: {
				OR: [
					{
						// Public albums not associated with any circle
						circleId: null,
					},
					{
						// Albums from public circles
						Circle: {
							isPrivate: false,
						},
					},
		...
		<>
			<main className='w-full max-w-xl mx-auto min-h-screen bg-background flex flex-col items-center px-2 pb-8'>
				...
				<section className='w-full mt-4'>
					{!isGuest && userCirclesFormatted.length > 0 ? (
						<>
						...
						<>
							{' '}
							<h2 className='text-lg font-bold mb-2'>{isGuest ? 'Popular Circles' : 'Suggested Circles'}</h2>
							{!isGuest && <p className='text-circles-light opacity-80 mb-3 text-sm'>You&apos;re not in any circles yet. Here are some you might like:</p>}
				...
				<section className='w-full my-8 mb-32'>
					<h2 className='text-lg font-bold mb-2'>{isGuest ? 'Recent Albums' : 'Your Feed'}</h2>
					{feedAlbums.length > 0 ? (
						...
						<div className='py-6 text-center bg-opacity-10 bg-circles-light rounded-lg'>
							<p className='text-circles-light mb-3'>No albums available</p>
							{!isGuest && (
								<>
